...
class ServerControls {
    ...
    fun disconnectAll() {
        session.logger.log("Avvio disconnessione degli utenti.", References.LEVEL_SERVER)
        Thread().run {
            ...
            session.logger.log("Tutti gli utenti sono stati disconnessi correttamente.", References.LEVEL_WARNING)
    ...
    fun log() {
        session.logger.log("Inserire testo da loggare nel file: ", References.LEVEL_MESSAGE)
        session.logger.log("# USER: " + session.scanner.nextLine(), References.LEVEL_FILE)
    }
}