...
class ConnectionHandler : Thread() {
    ...
    override fun run() {
        ...
        println("Avvio della gestione di: " + clientSocket.inetAddress.hostAddress,
            References.LEVEL_LOG
        )
        Thread.sleep(20000)
        clientSocket.close()
        println("Disconnessione da: " + clientSocket.inetAddress.hostAddress,
            References.LEVEL_LOG
        )
    }
    ...
    fun println(message: String, level: Int) {
        ...
        ServerSession.logger.log("[ " + String.format("%03d", Thread.currentThread().id) + " ] $message", level)
    }
}