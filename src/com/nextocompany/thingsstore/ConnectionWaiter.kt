...
class ConnectionWaiter : Thread() {
    ...
    override fun run() {
        ...
        session.logger.log("Attivato l'ascolto di nuove connessioni.", References.LEVEL_WARNING)
        ...
        while (session.serverStatus == References.STATUS_RUNNING) {
            ...
            try {
                ...
                if (session.serverStatus != References.STATUS_STOPPED) {
                    session.serverStatus = References.STATUS_ERROR
                    session.logger.log("Il server è stato interrotto senza apparente ragione.", References.LEVEL_ERROR)
                }
            }
        }

        session.logger.log("Disattivato l'ascolto di nuove connessioni.", References.LEVEL_WARNING)
    }
